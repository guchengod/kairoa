...
### 🤖 AI 聊天
- 多服务商支持：OpenAI、DeepSeek、Anthropic、Azure OpenAI 及自定义服务商
- 管理多个 API 配置，每个服务商可配置多个模型
- 历史会话管理（保存、加载、切换、删除会话）
- Markdown 渲染，代码块支持语法高亮
- 停止生成功能，可取消正在进行的回复
- API Key 本地加密存储，安全可靠
- 每个配置支持自定义系统提示词
- 实时流式响应
...
### 🔍 路由追踪
- 追踪数据包到达目标主机所经过的路由路径
- 显示每一跳的主机名、IP 地址和往返时间（RTT）
- 可配置最大跳数和超时设置
- 跨平台支持（Windows tracert，Linux/macOS traceroute）
- 适用于网络诊断、路由分析和连接故障排查
